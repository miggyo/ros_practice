...
class DistTurtleServer(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('almosts_goal_time', 0.95)

        (quantile_time, almosts_time) = self.get_parameters(['quantile_time', 'almosts_goal_time'])
    ...
    def parameter_callback(self, params):
        for param in params:
            print(param.name, " is changed", param.value)

            if param.name == 'quantile_time' :
                self.quantile_time = param.value
            if param.name == 'almosts_goal_time' :
